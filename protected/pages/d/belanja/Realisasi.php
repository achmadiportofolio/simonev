...
class Realisasi extends MainPageBelanja  {	
	...
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack && !$this->IsCallback) {	
            ...
            if (!isset($_SESSION['currentPageRealisasi'])||$_SESSION['currentPageRealisasi']['page_name']!='d.belanja.Realisasi') {
                $_SESSION['currentPageRealisasi']=array('page_name'=>'d.belanja.Realisasi','page_num'=>0,'dataUraian'=>array(),'viewindex'=>0);												
			}  
            ...
            if (isset($result[1])) {             
                ...
                $this->uraianAnchor->NavigateUrl=$this->Service->constructUrl('d.belanja.Uraian',array('id'=>$result[1]['idproyek']));
                $this->uraianAnchor2->NavigateUrl=$this->Service->constructUrl('d.belanja.Uraian',array('id'=>$result[1]['idproyek']));
                $this->realisasiAnchor->NavigateUrl=$this->Service->constructUrl('d.belanja.Realisasi',array('id'=>$iduraian));
                $this->cmbPhotoBulan->DataSource=$this->TGL->getMonth (1);
                $this->cmbPhotoBulan->dataBind();
                
                $tglinput = new DateTime(); // Today    
                $tanggalmulai=$this->setup->getSettingValue('config_tanggal_mulai_input');
                $tanggalselesai=$this->setup->getSettingValue('config_tanggal_selesai_input');
                $objTGLMulai = new DateTime(date("Y-m-$tanggalmulai"));
                $objTGLSelesai  = new DateTime(date("Y-m-$tanggalselesai"));
                if (!($tglinput->getTimestamp() > $objTGLMulai->getTimestamp() && $tglinput->getTimestamp() < $objTGLSelesai ->getTimestamp())){                  
                    $this->btnTambahRealisasi->Enabled=false;
                    $this->btnTambahRealisasi->Attributes->OnClick="alert('tidak bisa input karena diluar waktu yang telah ditentukan');";  
                }            
    			$this->populateData();
    ...
    public function panelViewChanged ($sender,$param) {
        ...
        $this->kegiatan->redirect('d.belanja.Realisasi',array('id'=>$iduraian));				
    }
    ...
    public function addProcess ($sender,$param) {
        ...
        $nilai_pagu_uraian=$_SESSION['currentPageRealisasi']['dataUraian']['nilai'];
        $totalfisik=$this->kegiatan->getTotalFisik($iduraian);
        $totalrencanatargetfisik=$this->DB->getSumRowsOfTable('fisik',"target_uraian WHERE iduraian=$iduraian");
        $bobot=($nilai_pagu_uraian>0)?number_format(($nilai_pagu_uraian/$_SESSION['currentPageRealisasi']['dataUraian']['nilai_pagu'])*100,2):0;        
        if (round($totalrencanatargetfisik,2) < round($bobot,2)) {
            $this->idProcess='edit';
            $this->jumlahBobot=$bobot;
        }else {
            $this->idProcess='add';
            ...
            $this->fisikSlider->MaxValue=$bobot-$totalfisik;        
            $this->fisikSlider->StepSize=0.01;
