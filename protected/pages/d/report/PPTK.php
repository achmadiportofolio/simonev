...
class PPTK extends MainPageReports {
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {
            ...
            if (isset($_SESSION['currentPageReportPPTK']['datakegiatan']['nip_pptk'])) {               
                $this->detailProcess();
            }else {                
                if (!isset($_SESSION['currentPageReportPPTK'])||$_SESSION['currentPageReportPPTK']['page_name']!='d.report.ReportPPTK') {
                    $_SESSION['currentPageReportPPTK']=array('page_name'=>'d.report.ReportPPTK','page_num'=>0,'datakegiatan'=>array());												
                }            
                $this->toolbarOptionsTahunAnggaran->Enabled=false;
                $this->literalHeader->Text='Laporan Kegiatan Berdasarkan PPTK ';
                $this->populateData ();			
            }
		}
	}   
	...
	protected function populateData () {		
        ...
        $result=array();
        $str = "SELECT nip_pptk,COUNT(idproyek) AS jumlah_proyek,SUM(nilai_pagu) AS nilai_pagu FROM proyek WHERE tahun_anggaran=$ta GROUP BY nip_pptk ORDER BY nip_pptk ASC";
        ...
        while (list($k,$v)=each($r)) {
            ...
            foreach ($kegiatan as $pejabat) {                
                if ($pejabat['nip_pptk']==$v['nip_pptk']) {
                    ...
                    $realisasi=$this->finance->toRupiah($this->DB->getSumRowsOfTable("realisasi","proyek pr,uraian u,penggunaan p WHERE pr.idproyek=u.idproyek AND u.iduraian=p.iduraian AND nip_pptk=$nip"));
                    break;
                }                
            }
            $v['jumlah_kegiatan']=$jumlah_kegiatan;
            $v['nilai_pagu']=$nilai_pagu;
            $v['realisasi']=$realisasi;
            $result[$k]=$v;
        }               
    ...
    public function viewRecord($sender,$param) {
        ...
        $str = "SELECT nip_pptk,nama_pptk FROM pptk WHERE nip_pptk=$nip";        
		$this->DB->setFieldTable(array('nip_pptk','nama_pptk'));
		$r=$this->DB->getRecord($str);                   
        $_SESSION['currentPageReportPPTK']['datakegiatan']=$r[1];
        $this->redirect('d.report.PPTK');
    }    
    public function detailProcess () {
        $this->idProcess='view';
        $datapejabat=$_SESSION['currentPageReportPPTK']['datakegiatan'];        
        $this->literalHeader->Text="Daftar Kegiatan yang Ditangani oleh {$datapejabat['nama_pptk']} ";
        $this->contentReport->Text=$this->printContent();
    }
    /**
	...
	*/
	private function getTotalNilaiPaguUnit ($idunit,$ta)  {
        $nip_pptk=$_SESSION['currentPageReportPPTK']['datakegiatan']['nip_pptk'];
		$str = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta' AND nip_pptk=$nip_pptk";				
	...
	*/
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.nama_proyek,p.nilai_pagu,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE idprogram='$idprogram' AND nip_pptk='{$datapejabat['nip_pptk']}'";
    ...
    public function printOutRekapitulasi ($sender,$param) {           
        $this->createObjReport();        
        $this->report->dataKegiatan=$_SESSION['currentPageReportPPTK']['datakegiatan'];
        $this->report->dataKegiatan['idunit']=$this->idunit;
        $this->report->dataKegiatan['tahun']=$_SESSION['ta'];                
        $filetype=$this->cmbTipePrintOutRekapitulasi->Text;        		
        switch($filetype) {
            case 'excel2003' :                				                
                $this->report->setMode('excel2003');
                $this->report->printRekapPPTK($this->linkOutputRekapitulasi);
            break;
            case 'excel2007' :				                
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan['idunit']=$this->idunit;
        $this->report->dataKegiatan['tahun']=$_SESSION['ta'];        
        $this->report->dataKegiatan['tipe_pejabat']='pptk';        
    ...
    public function close($sender,$param) {        
        unset($_SESSION['currentPageReportPPTK']);
        $this->redirect('d.report.PPTK');
    }
}
