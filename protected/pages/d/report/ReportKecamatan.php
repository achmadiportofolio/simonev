...
class ReportKecamatan extends MainPageReports {
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {
            ...
            if (isset($_SESSION['currentPageReportKecamatan']['datakegiatan']['idkecamatan'])) {
                ...
                if (!isset($_SESSION['currentPageReportKecamatan'])||$_SESSION['currentPageReportKecamatan']['page_name']!='d.report.ReportKecamatan') {
                    $_SESSION['currentPageReportKecamatan']=array('page_name'=>'d.report.ReportKecamatan','page_num'=>0,'datakegiatan'=>array());												
                }                            
	...
	protected function populateData () {	
        $ta=$this->session['ta'];
        $str = 'SELECT idkecamatan,nama_dt2,nama_kecamatan FROM kecamatan kec LEFT JOIN dt2 ON (kec.iddt2=dt2.iddt2) ORDER BY kec.iddt2 ASC';        
		$this->DB->setFieldTable(array('idkecamatan','nama_dt2','nama_kecamatan'));
		$r=$this->DB->getRecord($str);    
        $result=array();
        $str = "SELECT idlok,COUNT(idproyek) AS jumlah_proyek,SUM(nilai_pagu) AS nilai_pagu FROM proyek WHERE ket_lok='kec' AND tahun_anggaran=$ta GROUP BY idlok ORDER BY idlok ASC";
    ...
    public function viewRecord($sender,$param) {
        $idkecamatan=$this->getDataKeyField($sender, $this->RepeaterS);
        $str = "SELECT idkecamatan,nama_dt2,nama_kecamatan FROM kecamatan kec LEFT JOIN dt2 ON (dt2.iddt2=kec.iddt2) WHERE idkecamatan=$idkecamatan";        
		$this->DB->setFieldTable(array('idkecamatan','nama_dt2','nama_kecamatan'));
		$r=$this->DB->getRecord($str);            
        $_SESSION['currentPageReportKecamatan']['datakegiatan']=$r[1];
        $this->redirect('d.report.ReportKecamatan');
    }    
    public function detailProcess () {
        $this->idProcess='view';
        $datalokasi=$_SESSION['currentPageReportKecamatan']['datakegiatan'];        
        $this->literalHeader->Text="Daftar Kegiatan di {$datalokasi['nama_kecamatan']} - {$datalokasi['nama_dt2']}";
        $this->contentReport->Text=$this->printContent();
    }
    /**
	...
	*/
	private function getTotalNilaiPaguUnit ($idunit,$ta)  {
        $idkecamatan=$_SESSION['currentPageReportKecamatan']['datakegiatan']['idkecamatan'];
		$str = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta' AND ket_lok='kec' AND idlok=$idkecamatan";				
	...
	*/
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.nama_proyek,p.nilai_pagu,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE idprogram='$idprogram' AND ket_lok='kec' AND idlok='{$datalokasi['idkecamatan']}'";
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan=$_SESSION['currentPageReportKecamatan']['datakegiatan'];
        $this->report->dataKegiatan['idunit']=$this->idunit;
        $this->report->dataKegiatan['tahun']=$_SESSION['ta'];        
        $this->report->dataKegiatan['tipe_lokasi']='kec';        
    ...
    public function close($sender,$param) {        
        unset($_SESSION['currentPageReportKecamatan']);
        $this->redirect('d.report.ReportKecamatan');
    }
}
