...
class ReportDT1 extends MainPageReports {
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {            
            ...
            if (isset($_SESSION['currentPageReportDT1']['datakegiatan']['iddt1'])) {
                ...
                if (!isset($_SESSION['currentPageReportDT1'])||$_SESSION['currentPageReportDT1']['page_name']!='d.report.ReportDT1') {
                    $_SESSION['currentPageReportDT1']=array('page_name'=>'d.report.ReportDT1','page_num'=>0,'datakegiatan'=>array());												
                }              
                $this->literalHeader->Text='Laporan Kegiatan Berdasarkan Daerah Tingkat I ';
                $this->populateData ();			
            }
		}
	}
	...
	protected function populateData () {				
        $ta=$this->session['ta'];
        $str = 'SELECT iddt1,nama_dt1 FROM dt1 ORDER BY dt1.iddt1 ASC';        
		$this->DB->setFieldTable(array('iddt1','nama_dt1'));
		$r=$this->DB->getRecord($str);    
        $result=array();
        $str = "SELECT idlok,COUNT(idproyek) AS jumlah_proyek,SUM(nilai_pagu) AS nilai_pagu FROM proyek WHERE ket_lok='dt1' AND tahun_anggaran=$ta GROUP BY idlok ORDER BY idlok ASC";
    ...
    public function viewRecord($sender,$param) {
        $iddt1=$this->getDataKeyField($sender, $this->RepeaterS);
        $str = "SELECT iddt1,nama_dt1 FROM dt1 WHERE iddt1=$iddt1";        
		$this->DB->setFieldTable(array('iddt1','nama_dt1'));
		$r=$this->DB->getRecord($str);            
        $_SESSION['currentPageReportDT1']['datakegiatan']=$r[1];
        $this->redirect('d.report.ReportDT1');
    }    
    public function detailProcess () {
        $this->idProcess='view';
        $datalokasi=$_SESSION['currentPageReportDT1']['datakegiatan'];        
        $this->literalHeader->Text="Daftar Kegiatan di {$datalokasi['nama_dt1']} ";
        $this->contentReport->Text=$this->printContent();
    }
    /**
	...
	*/
	private function getTotalNilaiPaguUnit ($idunit,$ta)  {
        $iddt1=$_SESSION['currentPageReportDT1']['datakegiatan']['iddt1'];
		$str = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta' AND ket_lok='dt1' AND idlok=$iddt1";				
	...
	*/
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.nama_proyek,p.nilai_pagu,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE idprogram='$idprogram' AND ket_lok='dt1' AND idlok='{$datalokasi['iddt1']}'";
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan=$_SESSION['currentPageReportDT1']['datakegiatan'];
        $this->report->dataKegiatan['idunit']=$this->idunit;
        $this->report->dataKegiatan['tahun']=$_SESSION['ta'];        
        $this->report->dataKegiatan['tipe_lokasi']='dt1';        
    ...
    public function close($sender,$param) {        
        unset($_SESSION['currentPageReportDT1']);
        $this->redirect('d.report.ReportDT1');
    }
}
