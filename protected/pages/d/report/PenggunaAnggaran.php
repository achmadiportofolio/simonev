...
class PenggunaAnggaran extends MainPageReports {
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {
            ...
            if (isset($_SESSION['currentPageReportPenggunaAnggaran']['datakegiatan']['nip_pengguna_anggaran'])) {               
                $this->detailProcess();
            }else {                
                if (!isset($_SESSION['currentPageReportPenggunaAnggaran'])||$_SESSION['currentPageReportPenggunaAnggaran']['page_name']!='d.report.ReportPenggunaAnggaran') {
                    $_SESSION['currentPageReportPenggunaAnggaran']=array('page_name'=>'d.report.ReportPenggunaAnggaran','page_num'=>0,'datakegiatan'=>array());												
                }            
                $this->toolbarOptionsTahunAnggaran->Enabled=false;
                $this->literalHeader->Text='Laporan Kegiatan Berdasarkan Pengguna Anggaran ';
                $this->populateData ();			
            }
		}
	}   
	...
	protected function populateData () {		
        $ta=$this->session['ta'];
        $str = 'SELECT nip_pengguna_anggaran,nama_pengguna_anggaran FROM pengguna_anggaran ORDER BY nip_pengguna_anggaran ASC';        
		$this->DB->setFieldTable(array('nip_pengguna_anggaran','nama_pengguna_anggaran'));
		$r=$this->DB->getRecord($str);    
        $result=array();
        $str = "SELECT nip_pengguna_anggaran,COUNT(idproyek) AS jumlah_proyek,SUM(nilai_pagu) AS nilai_pagu FROM proyek WHERE tahun_anggaran=$ta GROUP BY nip_pengguna_anggaran ORDER BY nip_pengguna_anggaran ASC";
        $this->DB->setFieldTable(array('nip_pengguna_anggaran','jumlah_proyek','nilai_pagu'));
		$kegiatan=$this->DB->getRecord($str);    
        while (list($k,$v)=each($r)) {
            $jumlah_kegiatan=0;
            $nilai_pagu=0;
            foreach ($kegiatan as $pejabat) {                
                if ($pejabat['nip_pengguna_anggaran']==$v['nip_pengguna_anggaran']) {
                    $jumlah_kegiatan=$pejabat['jumlah_proyek'];
                    $nilai_pagu=$this->finance->toRupiah($pejabat['nilai_pagu']);
                    break;
                }                
            }
    ...
    public function viewRecord($sender,$param) {
        ...
        $str = "SELECT nip_pengguna_anggaran,nama_pengguna_anggaran FROM pengguna_anggaran WHERE nip_pengguna_anggaran=$nip";        
		$this->DB->setFieldTable(array('nip_pengguna_anggaran','nama_pengguna_anggaran'));
		$r=$this->DB->getRecord($str);                   
        $_SESSION['currentPageReportPenggunaAnggaran']['datakegiatan']=$r[1];
        $this->redirect('d.report.PenggunaAnggaran');
    }    
    public function detailProcess () {
        $this->idProcess='view';
        $datapejabat=$_SESSION['currentPageReportPenggunaAnggaran']['datakegiatan'];        
        $this->literalHeader->Text="Daftar Kegiatan yang Ditangani oleh {$datapejabat['nama_pengguna_anggaran']} ";
        $this->contentReport->Text=$this->printContent();
    }
    /**
	...
	*/
	private function getTotalNilaiPaguUnit ($idunit,$ta)  {
        $nip_pengguna_anggaran=$_SESSION['currentPageReportPenggunaAnggaran']['datakegiatan']['nip_pengguna_anggaran'];
		$str = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta' AND nip_pengguna_anggaran=$nip_pengguna_anggaran";				
	...
	*/
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.nama_proyek,p.nilai_pagu,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE idprogram='$idprogram' AND nip_pengguna_anggaran='{$datapejabat['nip_pengguna_anggaran']}'";
                    ...
                    if (isset($daftar_kegiatan[1])) {
                        ...
                        $content.= '<tr>				
                                ...
                                <td class="left">&nbsp;</td>					
                                <td class="right"><strong>'.$totalpagueachprograd.'</strong></td>
                                <td class="left">&nbsp;</td>
                                <td class="left">&nbsp;</td>
                                <td class="left">&nbsp;</td>
                                <td class="left">&nbsp;</td>
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan=$_SESSION['currentPageReportPenggunaAnggaran']['datakegiatan'];
        $this->report->dataKegiatan['idunit']=$this->idunit;
        $this->report->dataKegiatan['tahun']=$_SESSION['ta'];        
        $this->report->dataKegiatan['tipe_pejabat']='pengguna_anggaran';        
        $filetype=$this->cmbTipePrintOut->Text;        		
        switch($filetype) {
            case 'excel2003' :                				
                $this->report->setMode('excel2003');
                $this->report->printJabatan($this->linkOutput);
            break;
            case 'excel2007' :				
    ...
    public function close($sender,$param) {        
        unset($_SESSION['currentPageReportPenggunaAnggaran']);
        $this->redirect('d.report.PenggunaAnggaran');
    }
}
