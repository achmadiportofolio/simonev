...
class formA extends MainPageReports {	
	...
	public function onLoad ($param) {		
        ...
        if (!$this->IsPostBack&&!$this->IsCallBack) {
            if (isset($this->session['currentPageFormA']['dataKegiatan']['idproyek'])) {                
                ...
                if (!isset($_SESSION['currentPageFormA'])||$_SESSION['currentPageFormA']['page_name']!='t.report.formA') {
                    $_SESSION['currentPageFormA']=array('page_name'=>'t.report.formA','page_num'=>0,'dataKegiatan'=>array(),'idprogram'=>'none','search'=>false,'userid'=>'none');												
                }
    ...
    public function viewRecord ($sender,$param) {
        ...
        $this->redirect('t.report.formA');
                
    }
	private function initialization () {	
		$idproyek=$this->session['currentPageFormA']['dataKegiatan']['idproyek'];	        
		$str = "SELECT kp.nama_kuasa_pengguna,kp.nip_kuasa_pengguna,pp.nip_pptk,pp.nama_pptk FROM proyek p, kuasa_pengguna kp,pptk pp WHERE p.nip_kuasa_pengguna=kp.nip_kuasa_pengguna AND p.nip_pptk=pp.nip_pptk AND p.idproyek='$idproyek'";
	...
	private function getDpaKas ($idproyek,$bulan,$tahun) {
		...
		if (isset($this->result[1])) {
			...
			if ($bulan > '01') {
				...
				$str = "SELECT nilai,target,realisasi FROM v_laporan_a WHERE tahun_penggunaan='$tahun' AND idproyek='$idproyek'";				
				foreach ($r as $m=>$n) {
					$dpa=0;
					$kas=0;
...
//                                                $rp_total_realisasi_rek4=$this->finance->toRupiah($totalRealisasi_Rek4,"tanparp");
                                                ...
                                                foreach ($tingkat_5 as $k5=>$v5) {
                                                    if (ereg ($k4,$k5)) {
                                                        ...
                                                        $str = "SELECT iduraian,target,realisasi,CONCAT (volume,' ',satuan) AS volume FROM v_laporan_a WHERE bulan_penggunaan<='$no_bulan' AND tahun_penggunaan='$tahun' AND idproyek='$idproyek' AND no_rek5='$k5' ORDER BY bulan_penggunaan ASC";			
                                                        $result=$this->DB->getRecord($str);
                                                        $totalTarget2=0;
                                                        $totalRealisasi2=0;
                                                        ...
                                                        $url=$this->Service->constructUrl('t.report.formADetails',array('id'=>$iduraian));
