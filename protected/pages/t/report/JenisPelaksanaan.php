...
class JenisPelaksanaan extends MainPageReports {	
	...
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallback) {
            if (isset($this->session['currentPageReportJP']['dataKegiatan']['kode'])) {                
                ...
                if (!isset($_SESSION['currentPageReportJP'])||$_SESSION['currentPageReportJP']['page_name']!='t.report.JenisPelaksanaan') {
                    $_SESSION['currentPageReportJP']=array('page_name'=>'t.report.JenisPelaksanaan','page_num'=>0,'dataKegiatan'=>array());												
                }            
                $this->populateData ();
    ...
    protected function populateData () {   
        $ta=$this->session['ta'];
        $str ="SELECT jp,COUNT(iduraian) AS jumlah FROM uraian u,proyek p WHERE p.idproyek=u.idproyek AND status_lelang=1 AND p.tahun_anggaran=$ta GROUP BY jp";
	...
	public function viewRecord ($sender,$param) {
        ...
        $this->redirect('t.report.JenisPelaksanaan');        
	}
