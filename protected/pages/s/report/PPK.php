...
class PPK extends MainPageReports {
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {
            ...
            if (isset($_SESSION['currentPageReportPPK']['datakegiatan']['nip_ppk'])) {               
                $this->detailProcess();
            }else {                
                if (!isset($_SESSION['currentPageReportPPK'])||$_SESSION['currentPageReportPPK']['page_name']!='s.report.ReportPPK') {
                    $_SESSION['currentPageReportPPK']=array('page_name'=>'s.report.ReportPPK','page_num'=>0,'datakegiatan'=>array());												
                }            
                $this->toolbarOptionsTahunAnggaran->Enabled=false;
                $this->literalHeader->Text='Laporan Kegiatan Berdasarkan PPK ';
                $this->populateData ();			
            }
		}
	}   
	...
	protected function populateData () {		
        $ta=$this->session['ta'];        
        $str = 'SELECT nip_ppk,nama_ppk FROM ppk ORDER BY nip_ppk ASC';        
		$this->DB->setFieldTable(array('nip_ppk','nama_ppk'));
		$r=$this->DB->getRecord($str);    
        $result=array();
        $str = "SELECT nip_ppk,COUNT(idproyek) AS jumlah_proyek,SUM(nilai_pagu) AS nilai_pagu FROM proyek WHERE tahun_anggaran=$ta GROUP BY nip_ppk ORDER BY nip_ppk ASC";
        $this->DB->setFieldTable(array('nip_ppk','jumlah_proyek','nilai_pagu'));
		$kegiatan=$this->DB->getRecord($str);    
        while (list($k,$v)=each($r)) {
            $jumlah_kegiatan=0;
            $nilai_pagu=0;
            foreach ($kegiatan as $pejabat) {                
                if ($pejabat['nip_ppk']==$v['nip_ppk']) {
    ...
    public function viewRecord($sender,$param) {
        ...
        $str = "SELECT nip_ppk,nama_ppk FROM ppk WHERE nip_ppk=$nip";        
		$this->DB->setFieldTable(array('nip_ppk','nama_ppk'));
		$r=$this->DB->getRecord($str);                   
        $_SESSION['currentPageReportPPK']['datakegiatan']=$r[1];
        $this->redirect('s.report.PPK');
    }    
    public function detailProcess () {
        $this->idProcess='view';
        $datapejabat=$_SESSION['currentPageReportPPK']['datakegiatan'];        
        $this->literalHeader->Text="Daftar Kegiatan yang Ditangani oleh {$datapejabat['nama_ppk']} ";
        $this->contentReport->Text=$this->printContent();
    }
    /**
	...
	*/
	private function getTotalNilaiPaguUnit ($idunit,$ta)  {        
        $nip_ppk=$_SESSION['currentPageReportPPK']['datakegiatan']['nip_ppk'];
		$str = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta' AND nip_ppk=$nip_ppk";				
	...
	*/
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.nama_proyek,p.nilai_pagu,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE idprogram='$idprogram' AND nip_ppk='{$datapejabat['nip_ppk']}'";
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan=$_SESSION['currentPageReportPPK']['datakegiatan'];
        ...
        $this->report->dataKegiatan['tipe_pejabat']='ppk';        
    ...
    public function close($sender,$param) {        
        unset($_SESSION['currentPageReportPPK']);
        $this->redirect('s.report.PPK');
    }
}
