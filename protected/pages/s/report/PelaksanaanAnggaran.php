...
class PelaksanaanAnggaran extends MainPageReports {			
	public function onLoad ($param) {		
        ...
        if (!$this->IsPostBack&&!$this->IsCallBack) {
            if (!isset($_SESSION['currentPagePelaksanaanAnggaran'])||$_SESSION['currentPagePelaksanaanAnggaran']['page_name']!='s.report.PelaksanaanAnggaran') {
                $_SESSION['currentPagePelaksanaanAnggaran']=array('page_name'=>'s.report.PelaksanaanAnggaran','page_num'=>0,'listProgram'=>null);												
            }
	...
	public function printContent () {
        ...
        $str_pagu = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE userid=$userid AND p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta'";				
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.kode_proyek,p.nama_proyek,p.nilai_pagu,tk_hasil,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE userid=$userid AND idprogram='$idprogram'";
                    $daftar_kegiatan = $this->DB->getRecord($str);
                    if (isset($daftar_kegiatan[1])) {   
                        $str = "SELECT QUARTER(tanggal_realisasi) AS year_qtr,SUM(realisasi) AS total FROM v_laporan_a WHERE userid=$userid AND idprogram=$idprogram GROUP BY year_qtr";                                                                        
                        ...
                        while (list($m,$n)=each($daftar_kegiatan)) {
                            ...
                            $str = "SELECT QUARTER(tanggal_realisasi) AS year_qtr,SUM(realisasi) AS total FROM v_laporan_a WHERE userid=$userid AND idproyek=$idproyek GROUP BY year_qtr";                                                
                            $this->DB->setFieldTable(array('year_qtr','total'));
                            $realisasi=$this->DB->getRecord($str);              
