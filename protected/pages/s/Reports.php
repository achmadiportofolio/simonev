...
class Reports extends MainPageReports {
    ...
    public function onLoad($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {            
            if (!$this->IsPostBack&&!$this->IsCallBack) {
                if (!isset($_SESSION['currentPageReports'])||$_SESSION['currentPageReports']['page_name']!='s.Reports') {
                    $_SESSION['currentPageReports']=array('page_name'=>'s.Reports','page_num'=>0,'dataReport'=>array());												
                }
            }
    ...
    public function changeTahunAnggaran ($sender,$param) {	
        ...
        //$this->populateData();        
        $this->redirect('s.Reports');        
	}           
    protected function populateData () {
        $userid=$this->userid;
        $ta=$this->session['ta'];
        $str = "SELECT  jp, COUNT(iduraian) AS totaluraian FROM proyek p,uraian u WHERE p.idproyek=u.idproyek AND p.tahun_anggaran=$ta AND jp!='none' AND userid=$userid GROUP BY jp";
        ...
        $str = "SELECT  jp, COUNT(iduraian) AS totaluraian FROM proyek p,uraian u WHERE p.idproyek=u.idproyek AND p.tahun_anggaran=$ta AND jp!='none' AND u.status_lelang=1 AND userid=$userid GROUP BY jp";
        $this->DB->setFieldTable(array('jp','totaluraian'));
        $r=$this->DB->getRecord($str);               
        ...
        $str = "SELECT  jp, COUNT(iduraian) AS totaluraian FROM proyek p,uraian u WHERE p.idproyek=u.idproyek AND p.tahun_anggaran=$ta AND jp!='none' AND u.status_lelang=2 AND userid=$userid GROUP BY jp";
        $this->DB->setFieldTable(array('jp','totaluraian'));
        $r=$this->DB->getRecord($str);               
        ...
        $str = "SELECT  jp, COUNT(iduraian) AS totaluraian FROM proyek p,uraian u WHERE p.idproyek=u.idproyek AND p.tahun_anggaran=$ta AND jp!='none' AND u.status_lelang=0 AND userid=$userid GROUP BY jp";
        $this->DB->setFieldTable(array('jp','totaluraian'));
        $r=$this->DB->getRecord($str);               
