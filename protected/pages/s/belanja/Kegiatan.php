...
class Kegiatan extends MainPageBelanja {	    
	public function onLoad ($param) {		
		...
		if (!$this->IsCallback&&!$this->IsPostBack) {
            if (!isset($_SESSION['currentPageKegiatan'])||$_SESSION['currentPageKegiatan']['page_name']!='s.belanja.Kegiatan') {
                $_SESSION['currentPageKegiatan']=array('page_name'=>'s.belanja.Kegiatan','page_num'=>0,'idprogram'=>'none','search'=>false);												
			}            
            ...
            $daftar_user=$this->kegiatan->getList("user WHERE page='s'",array('userid','username'),'username',null,1);
	...
	protected function populateData ($search=false) {
        ...
        if ($search) {
            $str_jumlah="proyek WHERE userid=$userid AND tahun_anggaran=$tahun";
            $str_baris = "SELECT idproyek,kode_proyek,nama_proyek,nama_pptk FROM proyek p LEFT JOIN pptk ON (pptk.nip_pptk=p.nip_pptk) WHERE userid=$userid AND tahun_anggaran=$tahun";        
            ...
            $str_jumlah="proyek WHERE userid=$userid AND tahun_anggaran=$tahun $str_kode_program $str_kode_user";
            $str_baris = "SELECT idproyek,kode_proyek,nama_proyek,nama_pptk FROM proyek p LEFT JOIN pptk ON (pptk.nip_pptk=p.nip_pptk) WHERE userid=$userid AND tahun_anggaran=$tahun $str_kode_program $str_kode_user";        
        }
...
//                }
	...
	public function saveData ($sender,$param) {
		...
		if ($this->Page->IsValid) {				
            ...
            $this->kegiatan->redirect('s.belanja.Kegiatan');
		}
	}
