...
class Home extends MainPageDashboard {
    ...
    protected function populateData () { 
        $ta=$this->session['ta'];
        $str = "SELECT COUNT(DISTINCT(p.idproyek)) AS belumterealisasi FROM proyek p LEFT JOIN uraian u ON(p.idproyek=u.idproyek) LEFT JOIN penggunaan pe ON (pe.iduraian=u.iduraian) WHERE p.tahun_anggaran=$ta AND pe.realisasi IS NULL";
        $this->DB->setFieldTable(array('belumterealisasi'));
        $r=$this->DB->getRecord($str);
        $this->dataKegiatan['belumterealisasi']=$r[1]['belumterealisasi'];
        $this->dataKegiatan['totalkegiatan']=$this->DB->getCountRowsOfTable ("proyek WHERE tahun_anggaran=$ta",'idproyek');
        $this->dataKegiatan['terrealisasi']=$this->dataKegiatan['totalkegiatan']-$this->dataKegiatan['belumterealisasi'];
        $totalnilaipagu=$this->DB->getSumRowsOfTable('nilai_pagu',"proyek WHERE tahun_anggaran=$ta");
        ...
        $totalrealisasi=$this->DB->getSumRowsOfTable('realisasi',"v_laporan_a WHERE tahun_anggaran=$ta");
