...
class KuasaPengguna extends MainPageReports {
	public function onLoad ($param) {		
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {
            ...
            if (isset($_SESSION['currentPageReportKuasaPengguna']['datakegiatan']['nip_kuasa_pengguna'])) {               
                $this->detailProcess();
            }else {                
                if (!isset($_SESSION['currentPageReportKuasaPengguna'])||$_SESSION['currentPageReportKuasaPengguna']['page_name']!='m.report.ReportKuasaPengguna') {
                    $_SESSION['currentPageReportKuasaPengguna']=array('page_name'=>'m.report.ReportKuasaPengguna','page_num'=>0,'datakegiatan'=>array());												
                }            
                $this->toolbarOptionsTahunAnggaran->Enabled=false;
                $this->literalHeader->Text='Laporan Kegiatan Berdasarkan Kuasa Pengguna Anggaran ';
                $this->populateData ();			
            }
		}
	}   
	...
	protected function populateData () {		
        $ta=$this->session['ta'];
        $str = 'SELECT nip_kuasa_pengguna,nama_kuasa_pengguna FROM kuasa_pengguna ORDER BY nip_kuasa_pengguna ASC';        
		$this->DB->setFieldTable(array('nip_kuasa_pengguna','nama_kuasa_pengguna'));
		$r=$this->DB->getRecord($str);    
        $result=array();
        $str = "SELECT nip_kuasa_pengguna,COUNT(idproyek) AS jumlah_proyek,SUM(nilai_pagu) AS nilai_pagu FROM proyek WHERE tahun_anggaran=$ta GROUP BY nip_kuasa_pengguna ORDER BY nip_kuasa_pengguna ASC";
        $this->DB->setFieldTable(array('nip_kuasa_pengguna','jumlah_proyek','nilai_pagu'));
		$kegiatan=$this->DB->getRecord($str);    
        while (list($k,$v)=each($r)) {
            $jumlah_kegiatan=0;
            $nilai_pagu=0;
            foreach ($kegiatan as $pejabat) {                
                if ($pejabat['nip_kuasa_pengguna']==$v['nip_kuasa_pengguna']) {
    ...
    public function viewRecord($sender,$param) {
        ...
        $str = "SELECT nip_kuasa_pengguna,nama_kuasa_pengguna FROM kuasa_pengguna WHERE nip_kuasa_pengguna=$nip";        
		$this->DB->setFieldTable(array('nip_kuasa_pengguna','nama_kuasa_pengguna'));
		$r=$this->DB->getRecord($str);                   
        $_SESSION['currentPageReportKuasaPengguna']['datakegiatan']=$r[1];
        $this->redirect('m.report.KuasaPengguna');
    }    
    public function detailProcess () {
        $this->idProcess='view';
        $datapejabat=$_SESSION['currentPageReportKuasaPengguna']['datakegiatan'];        
        $this->literalHeader->Text="Daftar Kegiatan yang Ditangani oleh {$datapejabat['nama_kuasa_pengguna']} ";
        $this->contentReport->Text=$this->printContent();
    }
    /**
	...
	*/
	private function getTotalNilaiPaguUnit ($idunit,$ta)  {
        $nip_kuasa_pengguna=$_SESSION['currentPageReportKuasaPengguna']['datakegiatan']['nip_kuasa_pengguna'];
		$str = "SELECT SUM(p.nilai_pagu) AS total FROM proyek p,program pr WHERE p.idprogram=pr.idprogram AND pr.idunit='$idunit' AND pr.tahun='$ta' AND nip_kuasa_pengguna=$nip_kuasa_pengguna";				
	...
	*/
        ...
        if (isset($daftar_program[1])) {
            ...
            if ($totalPaguUnit > 0) {
                ...
                while (list($k,$v)=each($daftar_program)) {
                    ...
                    $str =  "SELECT p.idproyek,p.nama_proyek,p.nilai_pagu,p.sumber_anggaran,idlok,ket_lok FROM proyek p WHERE idprogram='$idprogram' AND nip_kuasa_pengguna='{$datapejabat['nip_kuasa_pengguna']}'";
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan=$_SESSION['currentPageReportKuasaPengguna']['datakegiatan'];
        $this->report->dataKegiatan['idunit']=$this->idunit;
        $this->report->dataKegiatan['tahun']=$_SESSION['ta'];        
        $this->report->dataKegiatan['tipe_pejabat']='kuasa_pengguna';        
    ...
    public function close($sender,$param) {        
        unset($_SESSION['currentPageReportKuasaPengguna']);
        $this->redirect('m.report.KuasaPengguna');
    }
}
