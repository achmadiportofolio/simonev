...
class formA extends MainPageReports {	
	...
	public function onLoad ($param) {		
        ...
        if (!$this->IsPostBack&&!$this->IsCallBack) {
            if (isset($_SESSION['currentPageFormA']['dataKegiatan']['idproyek'])) {                
                $this->idProcess='view';
                $this->initialization ();
            }else {
                if (!isset($_SESSION['currentPageFormA'])||$_SESSION['currentPageFormA']['page_name']!='m.report.formA') {
                    $_SESSION['currentPageFormA']=array('page_name'=>'m.report.formA','page_num'=>0,'dataKegiatan'=>array(),'idprogram'=>'none','search'=>false,'idunitkerja'=>'none');												
                }
                $_SESSION['currentPageFormA']['search']=false;
                $this->populateData ();		
            }
            $this->toolbarOptionsTahunAnggaran->DataSource=$this->TGL->getYear();
            $this->toolbarOptionsTahunAnggaran->Text=$_SESSION['ta'];
            $this->toolbarOptionsTahunAnggaran->dataBind();

            $this->toolbarOptionsBulanRealisasi->DataSource=$this->TGL->getMonth (3);
            $this->toolbarOptionsBulanRealisasi->Text=$_SESSION['bulanrealisasi'];
            $this->toolbarOptionsBulanRealisasi->dataBind();
            
            $this->cmbUnitKerja->DataSource=$this->kegiatan->getListUnitKerja();
            $this->cmbUnitKerja->Text=$_SESSION['currentPageFormA']['idunitkerja'];
    ...
    public function filterUnitKerja ($sender,$param) {
        $_SESSION['currentPageFormA']['idunitkerja']=$this->cmbUnitKerja->Text;
    ...
    protected function populateData ($search=false) {		
        $tahun=$_SESSION['ta'];        
        $bulan=$_SESSION['bulanrealisasi'];     
        $idunitkerja=$_SESSION['currentPageFormA']['idunitkerja'];
        $str_unitkerja=$idunitkerja=='none'?'':" AND pr.idunit=$idunitkerja";        
        if ($search) {
            $kriteria=$this->txtKriteria->Text;
            $str_jumlah="proyek p,program pr WHERE pr.idprogram=p.idprogram AND tahun_anggaran=$tahun $str_unitkerja";        
            $str_baris = "SELECT idproyek,kode_proyek,nama_proyek FROM proyek p,program pr WHERE pr.idprogram=p.idprogram AND tahun_anggaran=$tahun $str_unitkerja";                
