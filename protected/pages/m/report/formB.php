...
class formB extends MainPageReports {			
	public function onLoad ($param) {		
        ...
        if (!$this->IsPostBack&&!$this->IsCallBack) {
            if (!isset($_SESSION['currentPageFormB'])||$_SESSION['currentPageFormB']['page_name']!='m.report.formB') {
                $_SESSION['currentPageFormB']=array('page_name'=>'m.report.formB','page_num'=>0,'listProgram'=>null,'idunitkerja'=>'none');												
            }
            ...
            $this->cmbUnitKerja->Text=$_SESSION['currentPageFormB']['idunitkerja'];
            $this->cmbUnitKerja->dataBind();
            
            if ($_SESSION['currentPageFormB']['idunitkerja'] != 'none') {
                $this->populateProgramUnitKerja();                  
                $this->populateData ();
            }                             
        }
	}    
    ...
    public function filterUnitKerja ($sender,$param) {
        $idunitkerja=$this->cmbUnitKerja->Text;
        $_SESSION['currentPageFormB']['idunitkerja']=$idunitkerja;
        if ($idunitkerja=='none') {
            $this->contentReport->Text='';
            $this->footerReport->Text='';
            $this->listProgram->DataSource=array();
    ...
    private function populateProgramUnitKerja () {
        $tahun=$this->session['ta'];
        $idunit=$_SESSION['currentPageFormB']['idunitkerja'];            
	...
	*/
    ...
    public function printOut ($sender,$param) {   
        ...
        $this->report->dataKegiatan['idunit']=$_SESSION['currentPageFormB']['idunitkerja'];
        ...
        switch($filetype) {
            case 'excel2003' :                				
                ...
                $this->report->printFormB($this->linkOutput,$listProgram);
            break;
            case 'excel2007' :				
